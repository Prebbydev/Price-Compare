...
def load_kano_data():
    try:
        with open("kano_food.json", "r", encoding="utf-8") as file:
            return json.load(file)
    except FileNotFoundError:
        print("❌ Error: kano_food.json file not found!")
        return []
    except json.JSONDecodeError:
        print("❌ Error: Invalid JSON format in kano_food.json!")
        return []
...
def send_to_backend(data, batch_size=20):
    ...
    print(f"📤 Sending {len(new_data)} new kano data to the backend in batches of {batch_size}...")
...
if __name__ == "__main__":
    kano_data = load_kano_data()
    send_to_backend(kano_data)
    print("kano data transfer completed!")
